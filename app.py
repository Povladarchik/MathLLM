...
def parse_steps(text, patterns=None):
    ...
    steps = [part.strip() for part in parts if part.strip() and not re.match(f'{separator_pattern}\\s*\\d*', part)]
    return steps
...
def check_all_steps():
    ...
    for i, step in enumerate(steps):
        ...
        context = "\n".join([f"Шаг {j+i-3+1}: {steps[j]}" for j, step in enumerate(context_steps)])
